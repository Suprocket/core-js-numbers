...
function getAverage(value1, value2) {
  const output1 = value1 / 2;
  const output2 = value2 / 2;
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const firstHypotenuse = x2 - x1;
  const secondHypotenuse = y2 - y1;
  const newPoint = Math.sqrt(firstHypotenuse ** 2 + secondHypotenuse ** 2);
  return newPoint;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosAngle = xy / (moduleX * moduleY);
  const angleInRad = Math.acos(cosAngle);
  return angleInRad;
}
...
function roundToPowerOfTen(num, pow) {
  const numUpdated = Math.round(num / 10 ** pow);
  return numUpdated * 10 ** pow;
}
...
function getSumOfDigits(num) {
  let sum = 0;
  const itemOfNumArr = num.toString().split('');
  for (let i = 0; i < itemOfNumArr.length; i += 1) {
    sum += Number(itemOfNumArr[i]);
  }
  return sum;
}
...
function numberToStringInBase(number, base) {
  const hexString = number.toString(base);
  return hexString;
}
/**
